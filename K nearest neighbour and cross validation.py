...
def my_euclidean_dist(X_test, X_train):
	...
	for i in range(0,num_test):
    		a[i] = X_test[i][0]**2 + X_test[i][1]**2

	for j in range(0,num_train):
    		b[j] = X_train[j][0]**2 + X_train[j][1]**2


	for i in range(0,num_test):
    		for j in range(0,num_train):
        		dists[i][j] = (a[i] + b[j] + -2*X_test[i][0]*X_train[j][0] - 2*X_test[i][1]*X_train[j][1])
...
def find_k_neighbors(dists, Y_train, k):
  ...
  # idx will store variables in the form of indexes not the valus of dists
...
def knn_predict(X_test, X_train, Y_train, k):
	...
	for i in range(0,num_test):
		a = np.argmax(np.unique(neighbors[i],return_counts=True)[1])
		Y_pred[i] = np.unique(neighbors[i])[a]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    return Y_pred
...
def split_nfold(num_examples, n):
  ...
  # all the final remainder examples go to the final fold
	train_sets = []
	validation_sets = []
	...
	for i in range(0,n):
		...
		start = i*(num_examples//n)
		end = start + num_examples//n

		if(i == n-1):
			end = num_examples
    
		train_set = idx[0:start]+idx[end:num_examples]
		train_sets.append(train_set)
...
def cross_validation(classifier, X, Y, n, *args):
	...
	error_rate = np.sum(error_list)/n
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
	...
	for t in range(0,trials):
  		...
  		for i in range(0,nbatches):
    			for j in range(0,len(batches[i])):
      				ypred[batches[i][j]] = knn_predict(Xtest, trial_X, trial_Y,1)

  		error_rate[t] = compute_error_rate(yperd,ytest)

  		print("Trial number: ",trial_size," and error rate: ",error_rate[t])
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
err_ks = np.zeros(100)
for i in list_ks:
  	err_ks[i-1] = cross_validation(knn_predict,X_train, Y_train, 10, i)
best_k = np.argmin(err_ks) + 1
print("Best k is number: ",best_k,"\nAnd the error rate is: ",err_ks[best_k],"%\n\n\n")
...
final_error = compute_error_rate(y_pred, Y_test)
print("The error rate is: ",final_error,"%")
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
